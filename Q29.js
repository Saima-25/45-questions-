/* Favorite Fruit: Make an array of your favorite fruits, and then write a series of independent
if statements that check for certain fruits in your array.
 • Make an array of your three favorite fruits and call it favorite_fruits.

 • Write five if statements. Each should check whether a certain kind of fruit is in your array.
If the fruit is in your array, the if block should print a statement, such as You really like bananas!*/
// let favorite_fruits=['mango','orange','melon']
// if(favorite_fruits.includes('mango'))
//     {console.log('i like mango')}
// if(favorite_fruits.includes('melon'))
//     {console.log('i like melon')}
// if(favorite_fruits.includes('cherry'))
//     {console.log('i  dont like ')}
// if(favorite_fruits.includes('orange'))
//     {console.log('i like orange')}
// if(favorite_fruits.includes('apple'))
//     {console.log('i dont like apple')}
var favorite_fruits = ['mango', 'orange', 'melon'];
if (favorite_fruits.includes('mango')) {
    console.log('You really like mangoes!');
}
if (favorite_fruits.includes('orange')) {
    console.log('You really like oranges!');
}
if (favorite_fruits.includes('cherry')) {
    console.log('You really like cherries!');
}
if (favorite_fruits.includes('melon')) {
    console.log('You really like melons!');
}
if (favorite_fruits.includes('apple')) {
    console.log('You really like apples!');
}
